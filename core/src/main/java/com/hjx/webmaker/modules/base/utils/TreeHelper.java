...
public class TreeHelper {
     ...
     * @return
     */
    public static List<Node> getTreeNodes(List datas) {
    ...
    public static List<Node> getSortedNodes(List datas) {
        List<Node> result = new ArrayList<>();
        // 设置Node间父子关系
        List<Node> nodes = convertData2Node(datas);
        // 拿到根节点
    ...
    private static List<Node> convertData2Node(List nodes) {
...
//                    n.setParent(m);
    ...
    private static List<Node> getRootNodes(List nodes) {
        List<Node> root = new ArrayList<>();
        for (int i = 0; i < nodes.size(); i++) {
            Node node = (Node) nodes.get(i);
//            if (node.isRoot())
//                root.add(node);
            if (node.getParentId() != null && ("0".equals(node.getParentId().toString()))) {
                root.add(node);
            }
        }
        return root;
